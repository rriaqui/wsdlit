...
/**
 * Casos de proba para o filtrador de versións para DEVELOPMENT
 *
 * @author rriaqui
 * @since 1.0.0
 */
public class DevelopmentVersionFilterTestCase {
    @Test
    public void sameEnvironmentVersionsTest() {
        final List<Version> versions = Arrays.asList(
                ...
                new TestVersion( "1.9.0-SNAPSHOT" )
        );
        final String expected = "2.1-SNAPSHOT";
        final List<Version> filteredVersions = versions
                .stream()
                .filter( new VersionFilter( VersionType.DEVELOPMENT ) )
    ...
    public void onBottomTest() {
        final List<Version> versions = Arrays.asList(
                new TestVersion( "2.1.1-rc0" ),
                new TestVersion( "2.1.0" ),
                new TestVersion( "2.0.0-SNAPSHOT" )
        );
        final String expected = "2.0.0-SNAPSHOT";
        final List<Version> filteredVersions = versions
                .stream()
                .filter( new VersionFilter( VersionType.DEVELOPMENT ) )
                .collect( Collectors.toList() );
    ...
    public void onMiddleTest() {
        final List<Version> versions = Arrays.asList(
                new TestVersion( "2.1.0" ),
                new TestVersion( "2.1.0-SNAPSHOT" ),
                new TestVersion( "2.0.0" )
        );
        final String expected = "2.1.0-SNAPSHOT";
        final List<Version> filteredVersions = versions
                .stream()
                .filter( new VersionFilter( VersionType.DEVELOPMENT ) )
                .collect( Collectors.toList() );
    ...
    public void notFoundTest() {
        final List<Version> versions = Arrays.asList(
                new TestVersion( "2.1.0" ),
                new TestVersion( "2.0.5" ),
                new TestVersion( "2.0.5-rc0" ),
                new TestVersion( "2.0.0" )
        );
        final List<Version> filteredVersions = versions
                .stream()
