...
= ${document.title}
${organization}
v${document.version}, ${document.date?date?iso_utc}
//
...
// ICONS
:icon-disabled: icon:minus-circle[role=red]
:icon-enabled: icon:check-circle[role=green]
:icon-true: icon:check-circle[role=green]
:icon-false: icon:times-circle[role=red]
:icon-unsetted: icon:question-circle[role=silver]
:icon-bug: icon:bug[role=red,title=Bug]
:icon-delete: icon:trash[role=red,title=Eliminación]
:icon-new: icon:plus[role=green,title=Novo]
:icon-modify: icon:pencil[role=red,title=Modificación]
:icon-rename: icon:file[role=red,title=Cambio de nome]
:icon-version: icon:code-fork[role=red,title="Cambio de versión do esquema"]
:icon-cyclic-reference: icon:refresh[role=red,title="Referencia cíclica"]
