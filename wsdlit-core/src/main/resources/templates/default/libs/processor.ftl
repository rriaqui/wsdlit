...
|${counter}|<#compress>${element.fullName} <@icons.cyclicReferenceIcon element.isCyclicReference()/></#compress>|${element.occurrences}|<#if ! element.isModelGroup()>`${element.type}`</#if> ${(element.documentation?has_content)?string( element.documentation, "\n\nWARNING: Documentación ausente" )}
